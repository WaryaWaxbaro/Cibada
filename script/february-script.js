...
function headDetails(){
  ...
  var nDateLast = new Date(nYear + "-" + thisMonth + "-" + "28");
...
function printDoc(){
  ...
  printWindow.document.write('<title>February</title>');
...
xmlhttp.onreadystatechange = function() {
  if (this.readyState == 4 && this.status == 200) {
    ...
    for(var i = 0; i < jsonResult.length; i++){
      ...
      if(responseMonthId == 2){
        var resultMonthOfNov = jsonResult[i]["february"];
        //console.log(resultMonthOfNov);
        var monthDayId = resultMonthOfNov[0]["id"];
        var len = resultMonthOfNov.length-1;
        //console.log("len " + len);
